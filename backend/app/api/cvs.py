...
def analizar_en_background(proceso_id: int):
    ...
    try:
        ...
        get_logger(__name__).error("Error critico en background task: %s\n%s", e, traceback.format_exc())
    finally:
        db.close()
...
async def analizar(
    ...
    # Si forzar=True, resetear todos los analisis completados para re-procesar
    if forzar:
        ...
        for c in candidatos:
            ...
            if an:
                an.estado       = EstadoAnalisis.PENDIENTE
                an.puntaje_total = None
                an.detalle_json  = None
                an.resumen_ia    = None
                an.error_msg     = None
                an.progress_msg  = None
        db.commit()

    background_tasks.add_task(analizar_en_background, proceso_id)
    return {"mensaje": f"Análisis iniciado para {total} CV{'s' if total != 1 else ''}.", "total": total, "forzar": forzar}
